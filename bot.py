...
async def joined_required_channel(context, user_id):
    """Force-join checker. Only this function was changed.

    - Bot owner bypasses force-join.
    - Accepts @username, t.me/username, or numeric -100... channel ID.
    - Treats member/administrator/creator and active restricted members as joined.
    - Resolves the chat first and retries temporary Telegram/network failures.
    - Never treats a temporary timeout as a permanent non-member without retries.
    """
    if user_id == ADMIN_ID:
        return True
    ...
    # Normalize common channel formats without changing the Railway variable.
    chat_ref = raw
    if chat_ref.startswith("https://t.me/"):
        chat_ref = "@" + chat_ref[len("https://t.me/"):].split("/", 1)[0]
    elif chat_ref.startswith("http://t.me/"):
        chat_ref = "@" + chat_ref[len("http://t.me/"):].split("/", 1)[0]
    elif chat_ref.startswith("t.me/"):
        chat_ref = "@" + chat_ref[len("t.me/"):].split("/", 1)[0]
    elif chat_ref.startswith("https://telegram.me/"):
        chat_ref = "@" + chat_ref[len("https://telegram.me/"):].split("/", 1)[0]
    elif chat_ref.startswith("telegram.me/"):
        chat_ref = "@" + chat_ref[len("telegram.me/"):].split("/", 1)[0]
    ...
    for attempt in range(3):
        try:
            # Resolve the channel first so @username and numeric ID both work reliably.
            ...
            logger.info(
                "Force Join: user_id=%s status=%s chat=%s -> belum join",
                user_id, status, chat.id
            )
            return False
            ...
            last_error = e
            logger.warning(
                "Force Join check gagal (percobaan %s/3), channel=%s, user_id=%s: %s",
                attempt + 1, chat_ref, user_id, e
            )
            if attempt < 2:
                await asyncio.sleep(1.5)

    # Keep the previous safe behavior for a real Telegram/API failure, but log it clearly.
    logger.error(
        "Force Join tidak dapat diverifikasi setelah 3 percobaan. channel=%s user_id=%s error=%s",
        chat_ref, user_id, last_error
    )
    return False
